...
#     options.predictability = 8
    ...
    if options.model_data!=None:
        ...
        if (options.parameter == 'precipitation_1h_bg'):
            ...
            added_hours = 0
        image_array2, quantity2_min, quantity2_max, timestamp2, mask_nodata2, nodata2, longitudes2, latitudes2 = read(options.model_data, added_hours,use_as_template=True)
    ...
    if options.dynamic_nwc_data!=None:
        if (options.parameter == 'precipitation_1h_bg'):
            ...
            added_hours = 0
        image_arrayx1, quantityx1_min, quantityx1_max, timestampx1, mask_nodatax1, nodatax1, longitudesx1, latitudesx1 = read(options.dynamic_nwc_data,added_hours,use_as_template=True)
    ...
    write(interpolated_data=interpolated_advection,image_file=None,write_file=options.output_data,variable=options.parameter,predictability=options.predictability,t_diff=options.time_offset,grib_write_options=options.grib_write_options)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--grib_write_options",
                        type=str,
                        default=None,
