...
def optical_flow(R1_f, R2_f, farneback_params):
    if cv2.cuda.getCudaEnabledDeviceCount() > 0:
        ...
        gpu_flow = cv2.cuda_FarnebackOpticalFlow.create(
            numLevels, pyrScale, False, winSize, numIters, polyN, polySigma, flags
        )

        gpu_flow = cv2.cuda_FarnebackOpticalFlow.calc(
            gpu_flow, R1_f_gpu, R2_f_gpu, None
        )
...
def linear(
    ...
    n_interp_frames = (
        predictability * 3600 / seconds_between_steps - 1
    )  # predictability is defined in hours
    ...
    if (np.sum(R1 != missingval) == 0) | (np.sum(R2 != missingval) == 0):
        print(
            "either data field used for interpolation contains only missing values! Cannot interpolate and replacing intermediate timesteps with modelfield values!"
        )
...
def advection(
    ...
    R1_f = _filtered_ubyte_image(
        R1, mask_nodata, R_min, R_max, filter_stddev=1.0, logtrans=logtrans
    )
    R2_f = _filtered_ubyte_image(
        R2, mask_nodata, R_min, R_max, filter_stddev=1.0, logtrans=logtrans
    )
...
def model_smoothing(
    ...
    if predictability == None:
        raise ValueError(
            "You need to explicitly pass a predictability value to this function!"
        )
    # Do the motion vector calculation for each time step separately (using whole forecast length, PREDICTABILITY)
    all_fcst_lengths = list(
        range(1, predictability + 1)
    )  # range(1,(obsfields.shape[0])) Here, first forecast length has a non-zero coefficient and the last forecast length coefficient of one (only modelfield is used).
    ...
    tws = (
        logistic_growth_function(np.linspace(sigmoid_steepness, -0.7, n_interp_frames))
        / 0.5
    )
    ...
    for fcst_length in all_fcst_lengths:
        ...
        if (np.sum(R1 != missingval) == 0) | (np.sum(R2 != missingval) == 0):
            print(
                "either data field for forecast length "
                + str(fcst_length)
    ...
    # Masking in the previous step does not take into account that missing values can actually spread and dilute to actual field. Therefore appylying a simple check to remove spurious values from fields, based on absolute maximum/minimum values in either field.
    R_interp[R_interp > (R_max * 1.02)] = modelfields[
        R_interp > (R_max * 1.02)
    ]  # missingval
    R_interp[R_interp < (R_min * 0.98)] = modelfields[
        R_interp < (R_min * 0.98)
    ]  # missingval
...
def _filtered_ubyte_image(
    I, mask_nodata, R_min, R_max, filter_stddev=1.0, logtrans=False
):
    I = I.copy()
